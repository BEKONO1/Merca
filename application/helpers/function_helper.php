...
function fetch_details($table, $where = NULL, $fields = '*', $limit = '', $offset = '', $sort = '', $order = '', $where_in_key = '', $where_in_value = '', $like_key = '', $like_value = '')
{
    $t = &get_instance();
    $t->db->select($fields);
    if (!empty($where)) {
...
function fetch_product($user_id = NULL, $filter = NULL, $id = NULL, $category_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $return_count = NULL, $is_deliverable = NULL, $seller_id = NULL, $is_detailed_data = 0)
{
    ...
    $t->db->select($discount_filter_data . 'count(p.id) as sales, p.stock_type ,
     ...
     p.seo_page_title as product_seo_page_title,p.deliverable_group_type,p.deliverable_zipcodes_group,p.deliverable_cities_group,p.deliverable_city_group_type, p.seo_meta_keywords as product_seo_meta_keywords, p.seo_meta_description as product_seo_meta_description, p.seo_og_image as product_seo_og_image,
    ...
    if (!empty($product)) {
        ...
        for ($i = 0; $i < count($product); $i++) {
            ...
            $product[$i]['deliverable_zipcodes_group'] = isset($product[$i]['deliverable_zipcodes_group']) && !empty($product[$i]['deliverable_zipcodes_group']) ? output_escaping($product[$i]['deliverable_zipcodes_group']) : '';
            $product[$i]['deliverable_cities_group'] = isset($product[$i]['deliverable_cities_group']) && !empty($product[$i]['deliverable_cities_group']) ? output_escaping($product[$i]['deliverable_cities_group']) : '';
...
function create_consignment($data)
{
    ...
    // Send notification ONCE after consignment is created (moved outside the loop)
    if (!empty($user_fcm_ids) && !empty($user_fcm_ids[0]['fcm_id'])) {
        $settings = get_settings('system_settings', true);
        ...
        // Get unique FCM IDs only
        $unique_fcm_tokens = [];
        foreach ($user_fcm_ids as $item) {
            if (!empty($item['fcm_id']) && !in_array($item['fcm_id'], $unique_fcm_tokens)) {
                $unique_fcm_tokens[] = $item['fcm_id'];
            }
        }

        $fcm_count = count($unique_fcm_tokens);
        // $debug_info = " [DEBUG: Sending to " . $fcm_count . " unique FCM token(s) - Type: Parcel Created]";

        $customer_msg = (!empty($custom_notification)) ? $message : 'Hello Dear ' . $user_res[0]['username'] . ' Order status updated to processed for order ID #' . $order_id . ' please take note of it! Thank you. Regards ' . $app_name . '';
        // $customer_msg .= $debug_info;
        ...
        // Build FCM IDs array with unique tokens only
        $fcm_ids = [];
        foreach ($user_fcm_ids as $item) {
            if (!empty($item['fcm_id']) && in_array($item['fcm_id'], $unique_fcm_tokens)) {
                $platform = isset($item['platform_type']) && !empty($item['platform_type']) ? $item['platform_type'] : 'android';
                if (!isset($fcm_ids[$platform])) {
                    $fcm_ids[$platform] = [];
                }
                $fcm_ids[$platform][] = $item['fcm_id'];
                // Remove from unique list to avoid duplicates
                $unique_fcm_tokens = array_diff($unique_fcm_tokens, [$item['fcm_id']]);
            }
        }

        // Wrap in array format expected by send_notification
        foreach ($fcm_ids as $platform => $ids) {
            $fcm_ids[$platform] = [$ids];
        }
...
function view_all_consignments_old($order_id = null, $consignment_id = null, $seller_id = null, $offset = null, $limit = null, $order = "DESC", $in_detail = 1, $delivery_boy_id = null, $multiple_status = null, bool $show_otp = false)
{
    $t = &get_instance();
...
// Resolved an issue where the order total was displayed in parcels instead of the parcel-item total. do not change this code without permission of dikshita 
function view_all_consignments(
    $order_id = null,
    $consignment_id = null,
    $seller_id = null,
    $offset = null,
    ...
    $consignment_count = $count_res->get('consignments c')->row_array();
    $total = $consignment_count['total'] ?? 0;

    /**********************
     * FETCH CONSIGNMENTS
     **********************/
    $search_res = $t->db->select(
        'u.username,u.email,u.mobile,u.latitude,u.longitude,u.image as user_profile,
        u.id as user_id,c.id, c.order_id,c.delivery_boy_id,c.otp, c.name,
        c.status,c.active_status,o.date_added as created_date,
        ' . (($show_otp) ? "c.otp," : "") . '
        oi.seller_id as seller_id,o.payment_method,o.address as user_address,
        o.delivery_charge,o.wallet_balance,o.discount,o.promo_discount,o.total_payable,
        o.notes,o.delivery_date,o.delivery_time,o.is_cod_collected,o.is_shiprocket_order,
        o.final_total,o.total,o.address_id as user_address_id,
        o.mobile as order_mobile_number'
    )
        ->from('consignments c')
        ->join('consignment_items ci', 'ci.consignment_id = c.id', 'left')
        ->join('orders o', 'c.order_id = o.id', 'left')
    ...
    $result = $search_res->group_by("c.id")
        ->limit($limit, $offset)
        ->order_by("c.id", $order)
        ->get()
        ->result_array();
    ...
    foreach ($result as $row) {
         ...
         * BASIC DETAILS
         **********************/
        $seller_details = $t->db->select('sd.store_name,u.username as seller_name,u.address,u.mobile,
            sd.logo as store_image,u.latitude,u.longitude')
            ->from('seller_data sd')
            ->join('users u', 'u.id = sd.user_id')
            ->where('sd.user_id', $row['seller_id'])
            ->get()->row_array();
        ...
        $user_address = $t->db->select('a.id, a.name, a.latitude, a.longitude')
            ->from('addresses a')
            ->where('a.id', $row['user_address_id'])
            ->get()->row_array();

        if (!empty($delivery_boy_details['image']))
            $delivery_boy_details['image'] = base_url($delivery_boy_details['image']);
        else
            $delivery_boy_details['image'] = "";

        if (!empty($seller_details['store_image']))
            $seller_details['store_image'] = base_url($seller_details['store_image']);

        /**********************
         * TRACKING DETAILS
         **********************/
        $tracking_details = $t->db->select('*')
            ->from('order_tracking')
            ->where('consignment_id', $row['id'])
            ->where('is_canceled', 0)
            ->get()->row_array();

        $cancelled_tracking_details = $t->db->select('*')
            ->from('order_tracking')
            ->where('consignment_id', $row['id'])
            ->where('is_canceled', 1)
            ->get()->row_array();
        ...
        $data = [
            ...
            'user_id' => $row['user_id'] ?? '',
            'email' => $row['email'] ?? '',
            'mobile' => !empty($row['mobile']) ? $row['mobile'] : $row['order_mobile_number'],
            'order_id' => $row['order_id'] ?? '',
            'name' => $row['name'] ?? '',
            ...
            'is_cod_collected' => $row['is_cod_collected'],
            'is_shiprocket_order' => $row['is_shiprocket_order'],
            'is_shiprocket_order_created' => !empty($tracking_details) ? 1 : 0,
            'active_status' => $row['active_status'],
            'status' => json_decode($row['status'], true),
            'consignment_items' => [],
        ...
        foreach ($consignment_items as $item) {
            ...
            if (!empty($product_details)) {
                ...
                $orderData = $product_details['order_data'][0];

                $subtotal += $orderData['sub_total'];
                $total_tax_amount += $orderData['tax_amount'];
                $total_tax_percent += $orderData['tax_percent'];
        ...
        /**********************
         * DELIVERY CHARGE SPLIT (OPTION A)
         **********************/
        $total_consignments = $t->db->where("order_id", $row['order_id'])
            ->from("consignments")
            ->count_all_results();

        $delivery_charge_per_consignment = $row['delivery_charge'] / max($total_consignments, 1);

        $data['delivery_charge'] = number_format($delivery_charge_per_consignment, 2, '.', '');

        /**********************
         * DISTRIBUTE DISCOUNTS (Promo + Wallet)
         **********************/
        $share_percent = ($subtotal > 0 && $row['total'] > 0)
            ? ($subtotal / $row['total']) * 100
            : 0;

        $promo_discount = ($row['promo_discount'] * $share_percent) / 100;
        $wallet_balance = ($row['wallet_balance'] * $share_percent) / 100;

        $data['promo_discount'] = (string) round($promo_discount);
        $data['wallet_balance'] = (string) round($wallet_balance);
        ...
        $data['total'] = (string) round($subtotal - $promo_discount - $wallet_balance);

        $data['total_payable'] = (string) round(
            $subtotal +
            $delivery_charge_per_consignment -
            $promo_discount -
            $wallet_balance
        );

        $data['final_total'] = $data['total_payable'];
...
function send_digital_product_mail($to, $subject, $message, $attachment)
{
    ...
    if (empty($to) || !filter_var($to, FILTER_VALIDATE_EMAIL)) {
        $response['error'] = true;
        $response['message'] = 'Invalid recipient email address: ' . htmlspecialchars($to);
        $response['config'] = [];
        return $response;
    }
    ...
    // Validate and prepare attachment file
    if (!empty($attachment)) {
        // Convert relative path to server file path if needed
        $attachment_path = $attachment;
        
        // If attachment is a URL, extract the path part
        if (filter_var($attachment, FILTER_VALIDATE_URL)) {
            $url_parts = parse_url($attachment);
            $attachment_path = ltrim($url_parts['path'], '/');
            // Convert to full server path
            $attachment_path = FCPATH . $attachment_path;
        } else if (strpos($attachment, 'http://') === false && strpos($attachment, 'https://') === false) {
            // It's a relative path, convert to full server path
        ...
        if (!file_exists($attachment_path)) {
            $response['error'] = true;
            $response['message'] = 'Attachment file not found: ' . htmlspecialchars($attachment);
            $response['config'] = [];
            return $response;
        }
        
        $t->email->attach($attachment_path);
    }
...
function fetch_orders($order_id = NULL, $user_id = NULL, $multi_status = NULL, $delivery_boy_id = NULL, $limit = NULL, $offset = NULL, $sort = NULL, $order = NULL, $download_invoice = false, $start_date = null, $end_date = null, $search = null, $city_id = null, $area_id = null, $seller_id = null, $order_type = '', $from_seller = false, $draftFilter = 1)
{
    ...
    for ($i = 0; $i < count($order_details); $i++) {
        ...
        if (isset($order_details[$i]['attachments']) && !empty($order_details[$i]['attachments'])) {
            ...
            // Since the format is {"product_id": ["path1", "path2"]}, iterate through each product's images
            if (is_array($order_attachments)) {
                foreach ($order_attachments as $product_id => $attachment_paths) {
                    // Handle case where attachment_paths is an array of image paths
                    if (is_array($attachment_paths)) {
                        foreach ($attachment_paths as $image_path) {
                            $processed_attachments[] = get_image_url($image_path);
                        }
                    } else {
                        // Handle case where attachment_paths is a single image path
                        $processed_attachments[] = get_image_url($attachment_paths);
                    }
                }
            }
            $order_attachments = $processed_attachments;
        }
        ...
        for ($k = 0; $k < count($order_item_data); $k++) {
            ...
            if (!empty($order_item_data)) {
                ...
                // Process attachment field - it's stored as JSON with product IDs as keys and image paths as values
                if (isset($order_item_data[$k]['attachment']) && !empty($order_item_data[$k]['attachment'])) {
                    $attachment_data = json_decode($order_item_data[$k]['attachment'], true);
                    $processed_attachments = array();
                    if (is_array($attachment_data)) {
                        foreach ($attachment_data as $product_id => $attachment_paths) {
                            if (is_array($attachment_paths)) {
                                foreach ($attachment_paths as $image_path) {
                                    if (!empty($image_path)) {
                                        $processed_attachments[] = get_image_url($image_path);
                    ...
                    $order_item_data[$k]['attachment'] = $processed_attachments;
                } else {
                    $order_item_data[$k]['attachment'] = array();
                }
...
function get_sliders($id = '', $type = '', $type_id = '')
{
    ...
    $res = array_map(function ($d) {
        $ci = &get_instance();
        if ($d['type'] != "slider_url" && $d['type'] != "sliderurl") {
            $d['link'] = '';
        }
        if (!empty($d['type'])) {
        ...
        for ($i = 0; $i < $cart[0]['cart_count']; $i++) {
            ...
            if ($city_id > 0) {
                ...
                if (isset($settings['local_shipping_method']) && $settings['local_shipping_method'] == 1) {
                    ...
                    $tmpRow['is_deliverable'] = (!empty($zipcode_id) && $zipcode_id > 0) ?
                        is_product_delivarable('zipcode', $zipcode, $cart[$i]['product_id'])
                        : false;
                }
...
/**
 * Set and send email OTP for email verification
 * @param string $email User's email address
 * @param string $otp The OTP code to send
 * @return array Response with error status and message
 */
function set_email_otp($email, $otp)
{
    $t = &get_instance();
    $dateString = date('Y-m-d H:i:s');
    $time = strtotime($dateString);

    // Check if email already exists in otps table
    $otps = fetch_details('otps', ['email' => $email]);
    ...
    if (!empty($otps)) {
        ...
        $t->db->where('email', $email);
        $t->db->update('otps', $data);
    } else {
        // Insert new OTP record
        $data['email'] = $email;
        $data['mobile'] = '';
        $t->db->insert('otps', $data);
    }
    ...
    $subject = "Email Verification OTP - " . $app_name;
    $message = "
    <html>
    <head>
    ...
    <body style='font-family: Arial, sans-serif; padding: 20px;'>
        <div style='max-width: 600px; margin: 0 auto; background: #f9f9f9; padding: 30px; border-radius: 10px;'>
            ...
            <p style='color: #666; font-size: 14px; margin-top: 20px;'>This OTP is valid for 5 minutes. Please do not share this code with anyone.</p>
            <p style='color: #999; font-size: 12px; margin-top: 30px; text-align: center;'>If you did not request this verification, please ignore this email.</p>
            <p style='color: #999; font-size: 12px; text-align: center;'>© " . date('Y') . " {$app_name}. All rights reserved.</p>
        </div>
    </body>
    </html>";

    $mail_result = send_mail($email, $subject, $message);

    if ($mail_result['error'] == false) {
        return [
            "error" => false,
            "message" => "OTP sent successfully to your email.",
            "data" => ['otp' => $otp, 'created_at' => $time]
        ];
    } else {
        return [
            "error" => true,
...
function verify_email_otp($email, $otp)
{
    ...
    // Mark as verified
    update_details(['varified' => 1], ['email' => $email], 'otps');
...
function get_setup_wizard_progress()
{
    $CI = &get_instance();
    ...
    $modules = [
        ...
        [
            'key' => 'sellers',
            'name' => 'Sellers',
            'description' => 'Add seller accounts',
            'icon' => 'ti ti-users',
            'table' => 'users_groups',
        ...
        [
            'key' => 'delivery_boys',
            'name' => 'Delivery Boys',
            'description' => 'Add delivery personnel',
            'icon' => 'ti ti-truck-delivery',
            'table' => 'users_groups',
            'condition' => ['group_id' => 3],
            'link' => 'admin/delivery-boys/manage-delivery-boy'
        ],
        [
            'key' => 'zipcodes',
            'name' => 'Zipcodes',
            'description' => 'Setup delivery zones',
            'icon' => 'ti ti-map-pin',
            'table' => 'zipcodes',
            'link' => 'admin/area/manage-zipcodes'
        ],
        [
            'key' => 'cities',
            'name' => 'Cities',
            'description' => 'Configure available cities',
            'icon' => 'ti ti-building',
            'table' => 'cities',
            'link' => 'admin/area/manage-cities'
        ],
        [
            'key' => 'products',
            'name' => 'Products',
            'description' => 'Add product listings',
            'icon' => 'ti ti-package',
            'table' => 'products',
            'link' => 'admin/product'
        ]
    ...
    foreach ($modules as &$module) {
        if (isset($module['condition'])) {
            $CI->db->where($module['condition']);
        }

        $count = $CI->db->count_all_results($module['table']);
        $module['count'] = $count;
        $module['completed'] = ($count > 0);
    ...
    $percentage = ($total_modules > 0) ? round(($completed_count / $total_modules) * 100, 2) : 0;
    ...
    return [
        ...
        'has_incomplete' => ($completed_count < $total_modules)
    ];
}

function should_show_setup_wizard()
{
    $progress = get_setup_wizard_progress();
    return $progress['has_incomplete'];
}

function get_next_incomplete_module()
{
    $progress = get_setup_wizard_progress();
