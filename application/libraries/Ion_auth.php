...
class Ion_auth
{
	...
	public function __construct()
	{
        ...
        // Use the prefix calculated by the model
        $this->session_prefix = $this->ion_auth_model->session_prefix;
	}
	...
	public function logout()
	{
		...
		$this->session->unset_userdata([
			$this->session_prefix . $identity,
			$this->session_prefix . 'identity',
			$this->session_prefix . 'id',
			$this->session_prefix . 'user_id',
			$this->session_prefix . 'email',
			$this->session_prefix . 'old_last_login',
			$this->session_prefix . 'last_check',
			$this->session_prefix . 'ion_auth_session_hash'
		]);
		...
		// Destroy the session - removed to support simultaneous login
	...
	{
		$user_id = $this->session->userdata($this->session_prefix . 'user_id');
		// print_R($user_id);
		// die();
		if (!empty($user_id)) {
			return $user_id;
	...
	{
		$id = ($id == FALSE) ? $this->session->userdata($this->session_prefix . 'user_id') : $id;
