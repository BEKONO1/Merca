...
class Chat_model extends CI_Model
{
    ...
    {
        // Validate from_id to prevent SQL injection and malformed queries
        if (empty($from_id) || !is_numeric($from_id)) {
            return array();
        }
        ...
        $members = $this->db->select('m1.id, m1.from_id, m1.to_id, m1.is_read, m1.message, m1.type, m1.media, u.id AS opponent_user_id, u.image AS opponent_user_profile, u.username AS opponent_username, u.email, u.mobile, u.image, u.active, u.last_online')
            ->from('messages m1')
            ->join('users u', '(m1.from_id = ' . intval($from_id) . ' AND u.id = m1.to_id) OR (m1.to_id = ' . intval($from_id) . ' AND u.id = m1.from_id)', 'inner')
            ->where('m1.id = (SELECT MAX(m2.id) FROM messages m2 WHERE (m2.from_id = ' . intval($from_id) . ' OR m2.to_id = ' . intval($from_id) . ') AND ((m2.from_id = m1.from_id AND m2.to_id = m1.to_id) OR (m2.from_id = m1.to_id AND m2.to_id = m1.from_id)))')
            ->order_by('m1.id', 'desc')
            ->get()
            ->result_array();
