...
class Ion_auth_model extends CI_Model
{
    ...
    public function __construct()
    {
        ...
        if ($uri_segment_1 == 'admin') {
            $this->session_prefix = 'admin_';
        } elseif ($uri_segment_1 == 'seller') {
            $this->session_prefix = 'seller_';
        } elseif ($uri_segment_1 == 'delivery_boy' || $uri_segment_1 == 'delivery-boy') {
            $this->session_prefix = 'delivery_boy_';
        }  elseif ($uri_segment_1 == 'affiliate') {
            $this->session_prefix = 'affiliate_';
        }
    ...
    public function login($identity, $password, $remember = FALSE, $type = '')
    {
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($type == 'phone') {
                ...
                // SECURITY FIX: Verify password for email/other login types (not just phone)
                if ($this->verify_password($password, $user->password, $identity)) {
                    if ($user->active == 0) {
                        $this->trigger_events('post_login_unsuccessful');
                        $this->set_error('login_unsuccessful_not_active');
    ...
    {
        if (empty($this->session->userdata($this->session_prefix . 'identity'))) {
            return FALSE;
        }
        ...
        if ($recheck !== 0) {
            $last_login = $this->session->userdata($this->session_prefix . 'last_check');
            if ($last_login + $recheck < time()) {
                $query = $this->db->select('id')
                    ->where([
                        $this->identity_column => $this->session->userdata($this->session_prefix . 'identity'),
                        'active' => '1'
                    ])
                    ->limit(1)
                    ->order_by('id', 'desc')
                    ->get($this->tables['login_users']);
                if ($query->num_rows() === 1) {
                    $this->session->set_userdata($this->session_prefix . 'last_check', time());
                } else {
                    $this->trigger_events('logout');

                    $identity = $this->config->item('identity', 'ion_auth');

                    $this->session->unset_userdata([$this->session_prefix . $identity, $this->session_prefix . 'id', $this->session_prefix . 'user_id']);
        ...
        $session_hash = $this->session->userdata($this->session_prefix . 'ion_auth_session_hash');
    ...
    public function user($id = NULL)
    {
        ...
        $id = isset($id) ? $id : $this->session->userdata($this->session_prefix . 'user_id');
    ...
    public function get_users_groups($id = FALSE)
    {
        ...
        // if no id was passed use the current users id
        $id || $id = $this->session->userdata($this->session_prefix . 'user_id');
    ...
    public function add_to_group($group_ids, $user_id = FALSE)
    {
        ...
        // if no id was passed use the current users id
        $user_id || $user_id = $this->session->userdata($this->session_prefix . 'user_id');
    ...
    public function set_session($user)
    {
        ...
        $session_data = [
            $this->session_prefix . 'identity' => $user->{$this->identity_column},
            $this->session_prefix . $this->identity_column => $user->{$this->identity_column},
            $this->session_prefix . 'email' => $user->email,
            $this->session_prefix . 'user_id' => $user->id, //everyone likes to overwrite id so we'll use user_id
            $this->session_prefix . 'old_last_login' => $user->last_login,
            $this->session_prefix . 'last_check' => time(),
            $this->session_prefix . 'ion_auth_session_hash' => $this->config->item('session_hash', 'ion_auth'),
        ];
