...
class Home extends CI_Controller
{
	...
	public function index()
	{
		if ($this->ion_auth->logged_in() && $this->ion_auth->is_delivery_boy()) {
			...
			$user_res = $this->db->select('balance,bonus,username,bonus_type')->where('id', $user_id)->get('users')->result_array();
			...
			$this->data['bonus_type'] = ($user_res[0]['bonus_type'] == NULL) ? 'percentage_per_order_item' : $user_res[0]['bonus_type'];
