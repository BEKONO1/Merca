...
class Login extends CI_Controller
{
    ...
    public function update_user()
    {
        ...
        if (!empty($old) || !empty($new) || !empty($new_confirm)) {
            ...
            $this->form_validation->set_rules('new', $this->lang->line('change_password_validation_new_password_label'), 'required|xss_clean|min_length[8]|matches[new_confirm]|regex_match[/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&#])[A-Za-z\d@$!%*?&#]{8,}$/]', [
                'regex_match' => 'The password must be at least 8 characters long and contain a mix of uppercase, lowercase, numbers, and special characters.'
            ]);
            $this->form_validation->set_rules('new_confirm', $this->lang->line('change_password_validation_new_password_confirm_label'), 'required|xss_clean|matches[new]');
        }
        ...
        if (!preg_match($regex_latitude, $latitude)) {
            ...
            sendWebJsonResponse(true, 'Please enter a valid latitude.');
        }

        if (!preg_match($regex_longitude, $longitude)) {
            sendWebJsonResponse(true, 'Please enter a valid longitude.');
            return;
        }
        ...
        if (!$this->form_validation->run()) {
            ...
            if ($this->session->flashdata('message')) {
                sendWebJsonResponse(true, $this->session->flashdata('message'));
            }
        } else {
            if (!empty($old) || !empty($new) || !empty($new_confirm)) {
                $user_data = $this->ion_auth->user()->row();
                if (!$this->ion_auth->hash_password_db($user_data->id, $old)) {
                    sendWebJsonResponse(true, 'The old password is incorrect.');
                    return;
                }
            ...
            if ($profile_error != NULL) {
                sendWebJsonResponse(true, $profile_error);
            }
            ...
            sendWebJsonResponse(false, 'Profile Update Succesfully');
        }
    }
