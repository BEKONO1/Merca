...
class Point_of_sale extends CI_Controller
{
    ...
    public function register_user()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $message = (!empty($data)) ? "Registered Successfully" : "Not Registered";
            $data1 = (!empty($data)) ? $data : [];
            sendWebJsonResponse($error, $message, [], $data1);
        }
    ...
    public function place_order()
    {
        ...
        if (!isset($_POST['data']) || empty($_POST['data'])) {
            sendWebJsonResponse(true, 'Pass the data');
        }


        $post_data = json_decode($_POST['data'], true);
        if (!isset($_POST['user_id']) || empty($_POST['user_id'])) {
            sendWebJsonResponse(true, 'Please select the customer!');
        }
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_seller()) {
            if (isset($post_data) && !empty($post_data)) {
                for ($i = 0; $i < count($post_data); $i++) {
                    if (!isset($post_data[$i]['variant_id']) || empty($post_data[$i]['variant_id'])) {
                        sendWebJsonResponse(true, 'The variant ID field is required');
                    }

                    if (!isset($post_data[$i]['quantity']) || empty($post_data[$i]['quantity'])) {
                        sendWebJsonResponse(true, 'Please enter valid quantity for ' . $post_data[$i]['title']);
                    }
                }
            } else {
                sendWebJsonResponse(true, 'Pass the data');
