...
class Orders extends CI_Controller
{
    ...
    public function send_digital_product()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_seller() && ($this->ion_auth->seller_status() == 1 || $this->ion_auth->seller_status() == 0)) {
            ...
            // Fetch customer email from order using order_id
            $order_id = isset($_POST['order_id']) ? trim($_POST['order_id']) : null;
            if (!$order_id) {
                $this->response['error'] = true;
                $this->response['message'] = "Order ID is required.";
                $this->response['data'] = array();
            ...
            $order_data = fetch_details('orders', ['id' => $order_id], ['email']);

            if (empty($order_data) || empty($order_data[0]['email'])) {
                $this->response['error'] = true;
                $this->response['message'] = "Customer email not found in order.";
                $this->response['data'] = array();
                echo json_encode($this->response);
                return false;
            }

            $_POST['email'] = $order_data[0]['email'];
            ...
            if ($mail['error'] == true) {
                ...
                update_details(['status' => 'delivered'], ['order_id' => $order_id], 'order_items', false);
