...
*/
    ...
    protected $excluded_routes =
        [
            ...
            "seller/app/v1/api/get_zipcodes_group",
            "seller/app/v1/api/get_cities_group",
        ];
    ...
    */
        ...
        if (!$this->form_validation->run()) {
            ...
            if (!empty($products['product']) && !empty($seller_id)) {
                ...
                foreach ($products['product'] as &$product) {
                    if (isset($product['pickup_location']) && !empty($product['pickup_location'])) {
                        $pickup_location_id = $product['pickup_location'];
                        $pickup_details = fetch_details('pickup_locations', ['id' => $pickup_location_id], 'pickup_location');
                        $pickup_location_name = (!empty($pickup_details) && isset($pickup_details[0]['pickup_location'])) ? $pickup_details[0]['pickup_location'] : '';
                        $product['pickup_location'] = $pickup_location_id;
                        $product['pickup_location_name'] = $pickup_location_name;
                    }
                }
                unset($product);
    ...
    public function add_products()
    {
        /*
            ...
            deliverable_group_type:1        //{0:none, 1:all, 2:include, 3:exclude}
            deliverable_city_group_type:1        //{0:none, 1:all, 2:include, 3:exclude}
            deliverable_zipcodes:1,2,3  //{NULL: if deliverable_type = 0 or 1}
            deliverable_cities:1,2,3  //{NULL: if deliverable_type = 0 or 1}
            deliverable_cities_group:1,2,3  //{NULL: if deliverable_city_group_type = 0 or 1}
            deliverable_zipcodes_group:1,2,3  //{NULL: if deliverable_group_type = 0 or 1}
       ...
       */
        ...
        if (!$this->form_validation->run()) {
            ...
            $_POST['deliverable_cities_group'] = (!empty($_POST['deliverable_cities_group'])) ? $this->input->post('deliverable_cities_group', true) : NULL;
            $_POST['deliverable_zipcodes_group'] = (!empty($_POST['deliverable_zipcodes_group'])) ? $this->input->post('deliverable_zipcodes_group', true) : NULL;
    ...
    public function update_products()
    {
       ...
       */
        ...
        $this->form_validation->set_rules('made_in', 'Made In', 'trim|xss_clean');
        $this->form_validation->set_rules('hsn_code', 'HSN_Code', 'trim|xss_clean');
        $this->form_validation->set_rules('brand_id', 'Brand', 'trim|xss_clean');
        $this->form_validation->set_rules('product_type', 'Product type', 'trim|required|xss_clean');
        $this->form_validation->set_rules('total_allowed_quantity', 'Total Allowed Quantity', 'trim|xss_clean');
        $this->form_validation->set_rules('minimum_order_quantity', 'Minimum Order Quantity', 'trim|xss_clean');
        $this->form_validation->set_rules('quantity_step_size', 'Quantity Step Size', 'trim|xss_clean');
        ...
        if (!$this->form_validation->run()) {
            ...
            $_POST['brand'] = (isset($_POST['brand_id']) && !empty($_POST['brand_id'])) ? $_POST['brand_id'] : NULL;
    ...
    public function update_consignment_order_status()
    {
        ...
        */
        ...
        if (!$this->form_validation->run()) {
            ...
            $user_res = fetch_details('users', ['id' => $user_id], 'username,fcm_id,mobile,email,platform_type');
            $fcm_ids = array();
            //custom message - Send notification to customer
            if (!empty($user_res[0]['fcm_id'])) {
                if ($_POST['status'] == 'received') {
                    $type = ['type' => "customer_order_received"];
                } elseif ($_POST['status'] == 'processed') {
                ...
                // $debug_info = " [DEBUG: Status Update - Type: " . $_POST['status'] . "]";
                $customer_msg = (!empty($custom_notification)) ? $message : 'Hello Dear ' . $user_res[0]['username'] . 'Order status updated to' . $_POST['status'] . ' for order ID #' . $order_id . ' please take note of it! Thank you. Regards ' . $app_name . '';
                ...
                // Build FCM IDs array for single customer - only one notification needed
                $customer_platform = isset($user_res[0]['platform_type']) && !empty($user_res[0]['platform_type']) ? $user_res[0]['platform_type'] : 'android';
                $fcm_ids = [
                    $customer_platform => [[$user_res[0]['fcm_id']]]
                ];
