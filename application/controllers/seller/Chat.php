...
class Chat extends CI_Controller
{
    ...
    public function index()
    {
        if ($this->ion_auth->logged_in() && $this->ion_auth->is_seller() && ($this->ion_auth->seller_status() == 1 || $this->ion_auth->seller_status() == 0)) {
            ...
            $users = $this->chat_model->get_chat_history($this->ion_auth->get_user_id(), 10, 0);
            $user = array();
            $i = 0;
            $type = 'person';
    ...
    public function send_msg()
    {
        ...
        if (!$this->ion_auth->logged_in()) {
            ...
            $messages = $this->chat_model->get_msg_by_id($msg_id, $this->input->post('opposite_user_id'), $this->ion_auth->get_user_id(), $this->input->post('chat_type'));
            $message = array();
            $i = 0;
            ...
            if (!empty($msg_id)) {
                ...
                if (($this->input->post('chat_type') == 'person')) {
                    ...
                    $senders_info = fetch_details('users', ['active' => 1, 'id' => $this->ion_auth->get_user_id()]);
