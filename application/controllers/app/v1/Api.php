...
*/
    ...
    [
        ...
        "app/v1/api/reset_password_with_email",
        "app/v1/api/verify_user",
        "app/v1/api/get_login_identity",
        "app/v1/api/register_user",
        "app/v1/api/register_user_with_email",
        "app/v1/api/send_email_otp",
        "app/v1/api/verify_email_otp",
        ...
        "app/v1/api/get_zipcodes_group",
        "app/v1/api/get_cities_group",
    ...
    public function __construct()
    {
        ...
        if (!in_array($current_uri, $this->excluded_routes)) {
            ...
            $is_active = fetch_details('users', ['id' => $user_id], 'active');
            if ($is_active[0]['active'] == 0) {
                $response['error'] = true;
                $response['message'] = 'user_deactivated';
                $this->output->set_content_type('application/json')->set_status_header(403)->set_output(json_encode($response))->_display();
                die();
            }
        }
    }
  ...
  */
    ...
    public function login()
    {
        ...
        */
        ...
        $login = $this->ion_auth->login($identity, $this->input->post('password'), false);
        ...
        // Check if user is trying to login with mobile but hasn't verified their mobile number
        if (isset($_POST['mobile']) && !empty($_POST['mobile'])) {
            // Check if mobile_verified field exists and is not verified, or if user signed up with email
            if ((isset($user_data[0]['mobile_verified']) && $user_data[0]['mobile_verified'] == 0) ||
                (isset($user_data[0]['mobile_verified']) && $user_data[0]['mobile_verified'] == '0')
            ) {
                $response['error'] = true;
                $response['message'] = 'First verify your mobile, and after that you can log in using mobile';
                echo json_encode($response);
                return false;
            }
        }
        ...
        */
        ...
        if (isset($_POST['fcm_id']) && $_POST['fcm_id'] != NULL && !empty($_POST['fcm_id']) && $_POST['fcm_id'] != '-') {
            ...
            if ($is_logout == 1 || $is_logout == '1') {
                ...
                update_details(['fcm_id' => ''], ['id' => $user_id], 'users');
                $response['error'] = false;
        ...
        */
        ...
        if (!$this->form_validation->run()) {
            ...
            if (!$this->ion_auth->is_admin($user_data[0]['id']) && !$this->ion_auth->is_seller($user_data[0]['id']) && !$this->ion_auth->is_delivery_boy($user_data[0]['id'])) {
                ...
                if (isset($_POST['is_forgot_password']) && ($_POST['is_forgot_password'] == 1) && !is_exist(['mobile' => $_POST['mobile'], 'country_code' => $country_code], 'users')) {
                    $this->response['error'] = true;
                ...
                if (isset($_POST['mobile']) && is_exist(['mobile' => $_POST['mobile'], 'country_code' => $country_code], 'users') && (($_POST['is_forgot_password'] == 0) && isset($_POST['is_forgot_password']))) {
                    $this->response['error'] = true;
    ...
    public function register_user()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            $mobile = $this->input->post('mobile', true);
            $country_code = $this->input->post('country_code', true);
            if (is_exist(['mobile' => $mobile, 'country_code' => $country_code], 'users')) {
                $this->response['error'] = true;
                $this->response['message'] = 'The mobile number is already registered . Please login';
                $this->response['data'] = array();
                print_r(json_encode($this->response));
                return false;
            }
    ...
    public function update_user()
    {
        ...
        */
        ...
        $user_data = fetch_details('users', ['id' => $user_id], ['email', 'mobile', 'email_verified', 'mobile_verified']);

        // Check if user is trying to change verified email or mobile
        if (isset($_POST['email']) && !empty($_POST['email']) && $_POST['email'] != $user_data[0]['email']) {
            if (isset($user_data[0]['email_verified']) && $user_data[0]['email_verified'] == 1) {
                $response['error'] = true;
                $response['message'] = 'Cannot change verified email address';
                echo json_encode($response);
                return false;
            }
        }

        if (isset($_POST['mobile']) && !empty($_POST['mobile']) && $_POST['mobile'] != $user_data[0]['mobile']) {
            if (isset($user_data[0]['mobile_verified']) && $user_data[0]['mobile_verified'] == 1) {
                $response['error'] = true;
                $response['message'] = 'Cannot change verified mobile number';
                echo json_encode($response);
                return false;
            }
        }
        ...
        if (!$this->form_validation->run()) {
            ...
            $allowedFields = ['username', 'email', 'dob', 'mobile', 'address', 'city', 'area', 'pincode', 'latitude', 'longitude', 'country_code'];
            ...
            if ($user_data) {
                ...
                if ($login) {
                    ...
                    if ($user_group[0]['group_id'] == '2') {
                        ...
                        $response['error'] = true;
                        $response['message'] = 'The password you entered is incorrect. Please try again to confirm account deletion.';
                    }
                } else {
                    $response['error'] = true;
                    $response['message'] = 'The password you entered is incorrect. Please try again to confirm account deletion.';
    ...
    */
    ...
    /**
     * Send OTP to email for registration or verification
     * Parameters:
     *   email: user@example.com
     *   type: registration | forgot_password | verification (optional, default: registration)
     */
    public function send_email_otp()
    {
        ...
        $type = $this->input->post('type', true) ?: 'registration';

        // For registration, check if email is already registered
        if ($type == 'registration') {
            if (is_exist(['email' => $email], 'users')) {
                $this->response['error'] = true;
                $this->response['message'] = 'Email is already registered. Please login!';
                print_r(json_encode($this->response));
                return false;
            }
        }
        ...
        if ($type == 'forgot_password') {
            if (!is_exist(['email' => $email], 'users')) {
                $this->response['error'] = true;
                $this->response['message'] = 'Email is not registered. Please register first!';
                print_r(json_encode($this->response));
                return false;
            }
        }
     ...
     *   friends_code: ABCD1234 (optional)
     */
    public function register_user_with_email()
    {
        $this->form_validation->set_rules('name', 'Name', 'trim|required|xss_clean');
        ...
        if ($this->input->post('friends_code', true)) {
            ...
            if (count($used_refer_and_earn) < $refer_earn_bonus_times) {
                if (!is_exist(['referral_code' => $this->input->post('friends_code', true)], 'users')) {
                    $this->response['error'] = true;
                    $this->response['message'] = 'Invalid friends code!';
                    $this->response['data'] = array();
                    print_r(json_encode($this->response));
                    return false;
                }
                ...
                $this->response['error'] = true;
                $this->response['message'] = "Code already used for $refer_earn_bonus_times times!";
                $this->response['data'] = array();
                print_r(json_encode($this->response));
                return false;
            }
        ...
        $additional_data = [
            ...
            'mobile_verified' => 0, // Mobile needs to be verified separately
        ];
        ...
        if ($res) {
            ...
            // Clear OTP record
            delete_details(['email' => $email], 'otps');

            // Prepare response data
            unset($data[0]['password']);
            $data[0]['image'] = empty($data[0]['image']) || !file_exists(FCPATH . USER_IMG_PATH . $data[0]['image']) ? base_url() . NO_USER_IMAGE : base_url() . USER_IMG_PATH . $data[0]['image'];
    ...
    public function login_with_email()
    {
        ...
        if ($user_data[0]['active'] == 0) {
            $this->response['error'] = true;
            $this->response['message'] = 'Your account is inactive. Please contact support.';
            print_r(json_encode($this->response));
            return false;
        }
        ...
        if (isset($user_data[0]['email_verified']) && $user_data[0]['email_verified'] == 0) {
            $this->response['error'] = true;
            $this->response['message'] = 'Please verify your email address before logging in.';
            print_r(json_encode($this->response));
            return false;
        }

        // Get the user's actual type for login
        $login_type = isset($user_data[0]['type']) ? $user_data[0]['type'] : 'email';

        // Set identity column to email for login
        $this->ion_auth_model->identity_column = 'email';

        $login = $this->ion_auth->login($email, $this->input->post('password'), false, $login_type);

        if ($login) {
            if (!$this->ion_auth->is_admin() && !$this->ion_auth->is_seller() && !$this->ion_auth->is_delivery_boy()) {

                if (isset($_POST['fcm_id']) && !empty($_POST['fcm_id'])) {
                    update_details(['fcm_id' => $_POST['fcm_id']], ['email' => $email], 'users');
                }

                $data = fetch_details('users', ['email' => $email]);
                $existing_token = ($data[0]['apikey'] !== null && !empty($data[0]['apikey'])) ? $data[0]['apikey'] : "";
                unset($data[0]['password']);

                if ($existing_token == '') {
                    $token = generate_token($data[0]['mobile']);
                ...
                $this->response['token'] = isset($token) ? $token : $existing_token;
                $this->response['data'] = $rows;
            } else {
                $this->response['error'] = true;
    ...
    /**
     * Reset password with email (forgot password)
     * Parameters:
     *   email: user@example.com
     *   otp: 123456
    ...
    public function reset_password_with_email()
    {
        ...
        if (empty($user_data)) {
            $this->response['error'] = true;
            $this->response['message'] = 'Email is not registered.';
            print_r(json_encode($this->response));
            return false;
        }
        ...
        // Update password
        $identity_column = $this->config->item('identity', 'ion_auth');
        $identity = $user_data[0][$identity_column];

        $update = $this->ion_auth->reset_password($identity, $new_password);
    ...
    /**
     * Verify email from profile (for logged in users)
     * Parameters:
     *   email: user@example.com (must match logged in user's email)
     */
    public function send_profile_email_verification_otp()
    {
        if (!$this->verify_token()) {
            return false;
        }
    ...
    /**
     * Verify email OTP from profile (for logged in users)
     * Parameters:
     *   email: user@example.com
     *   otp: 123456
     */
    public function verify_profile_email()
    {
        if (!$this->verify_token()) {
            return false;
        }
        ...
        if ($result['error'] == false) {
            ...
            update_details(['email_verified' => 1], ['id' => $user_id], 'users');
    ...
    /**
     * Verify mobile from profile (for logged in users - after Firebase verification)
     * Parameters:
     *   mobile: 9876543210
     */
    public function verify_profile_mobile()
    {
        ...
        update_details(['mobile' => $mobile], ['id' => $user_id], 'users');

        // Update mobile_verified in users table
        update_details(['mobile_verified' => 1], ['id' => $user_id], 'users');
    ...
    public function get_verification_status()
    {
        ...
        $user = fetch_details('users', ['id' => $user_id], 'email_verified, mobile_verified');

        if (!empty($user)) {
            $this->response['error'] = false;
            $this->response['message'] = 'Verification status retrieved successfully.';
            $this->response['data'] = [
                'email_verified' => isset($user[0]['email_verified']) ? (int) $user[0]['email_verified'] : 0,
                'mobile_verified' => isset($user[0]['mobile_verified']) ? (int) $user[0]['mobile_verified'] : 0
            ];
        } else {
            $this->response['error'] = true;
            $this->response['message'] = 'User not found.';
    ...
    public function get_zipcodes_group()
    {
        $this->form_validation->set_rules('limit', 'limit', 'trim|numeric|xss_clean');
        $this->form_validation->set_rules('offset', 'offset', 'trim|numeric|xss_clean');
        $this->form_validation->set_rules('search', 'search', 'trim|xss_clean');
        ...
        if (!$this->form_validation->run()) {
            ...
            $data = $this->Area_model->get_zipcode_groups($search, $limit, $offset);
            $total = $this->db->count_all('zipcode_groups');
    ...
    public function get_cities_group()
    {
        ...
        */
        ...
        if (!$this->form_validation->run()) {
            ...
            $result = $this->Area_model->get_city_groups($search, $limit, $offset);
