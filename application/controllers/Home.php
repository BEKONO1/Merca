...
class Home extends CI_Controller
{
    ...
    public function login()
    {
        ...
        if ($this->form_validation->run() === TRUE) {
            ...
            // Check verification status based on login type
            if (!empty($user_data)) {
                if (preg_match($regex, $_POST['identity'])) {
                    ...
                    if (isset($user_data[0]['mobile_verified']) && $user_data[0]['mobile_verified'] == 0) {
                        $response['error'] = true;
                        $response['message'] = 'Please verify your mobile number before logging in.';
                        echo json_encode($response);
                        return false;
                    }
                }
            ...
            if (!empty($user_data) && isset($user_data[0]['type'])) {
                $login_type = $user_data[0]['type'];
            }
            ...
            if (!empty($res)) {
                ...
                if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember, $login_type)) {
                    ...
                    if (preg_match($regex, $_POST['identity'])) {
                        ...
                        $this->response['message'] = "Incorrect Email or password";
                    } else {
                        // User is logging in with phone number
    ...
    public function send_contact_us_email_old()
    {
